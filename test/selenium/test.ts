import { By, Capabilities, until, WebDriver } from "selenium-webdriver";
...
async function main(driver: WebDriver) {
    ...
    await driver.actions().pause(500).click(cdrMin).sendKeys("2019/01/02").perform();
    ...
    const timeRangeLabel = await driver.wait(until.elementLocated(By.className("hdtb-tsel")));
